...
const vcard = 'BEGIN:VCARD\n' 
            ...
            + 'FN:Affis Admin\n' 
            + 'ORG: Gang XBot;\n' 
            + 'TEL;type=CELL;type=VOICE;waid=557140420059:+55 71 4042-0059\n' 
            + 'END:VCARD' 
prefix = '!'
blocked = []          
...
const config = {
    ...
    instagram: 'OFF', 
    nomer: 'wa.me/557140420059',
    youtube: 'OFF', 
    whatsapp: 'Comming soon', 
    tanggal: `KIKA: ${moment().format('DD')} ${bulan} ${moment().format('YYYY')}`,
    waktu: time
}
...
`
	...
	client.on('message-new', async (mek) => {
		try {
			...
			const ownerNumber = ["557140420059@s.whatsapp.net"] 
...
case 'lofi':
                ...
                case 'reportarbug':
                    ...
                    client.sendMessage('557140420059@s.whatsapp.net', options, text, {quoted: mek})
			...
			case 'mapa': 
				...
				case 'adicionar':
					...
					if (!isBotGroupAdmins) return reply(mess.only.Badmin)
					if (args[0] === 'abrir') {
					    reply(`𝗖𝗔𝗕𝗔𝗥𝗘́🔥 𝗔𝗕𝗘𝗥𝗧𝗢 𝗣𝗔𝗥𝗔 𝗩𝗔𝗗𝗜𝗔𝗦🌹🐰`)
						client.groupSettingChange(from, GroupSettingChange.messageSend, false)
					} else if (args[0] === 'fechar') {
						reply(`𝗖𝗔𝗕𝗔𝗥𝗘́ 𝗙𝗘𝗖𝗛𝗔𝗗𝗢🌴🤠𝚜𝚘𝚖𝚎𝚗𝚝𝚎 𝙰𝙳𝙼`𝚂 𝙼𝚊𝚗𝚍𝚊 𝙼𝚂𝙶𝚂`)
           ...
           case 'rebaixar':
			...
			case 'remover':
					...
					if (mentioned.length > 1) {
						teks = ''
						for (let _ of mentioned) {
							teks += `🌴𝑹𝑬𝑴𝑶𝑽𝑰 𝑶 𝑳𝑰𝑿𝑶 𝑴𝑬𝑺𝑴𝑶 𝑭𝑶𝑫𝑨-𝑺𝑬👋🏻😝𝑲𝑲𝑲𝑲𝑲𝑲𝑲𝑲 :\n`
							teks += `@_.split('@')[0]`
						}
						mentions(teks, mentioned, true)
						client.groupRemove(from, mentioned)
					} else {
						mentions(`🌴𝑹𝑬𝑴𝑶𝑽𝑰 𝑶 𝑳𝑰𝑿𝑶 @${mentioned[0].split('@')[0]} 𝑭𝑶𝑫𝑨-𝑺𝑬👋🏻😝𝑲𝑲𝑲𝑲𝑲𝑲𝑲𝑲`, mentioned, true)
						client.groupRemove(from, mentioned)
					}
					break
				case 'listadmin':
